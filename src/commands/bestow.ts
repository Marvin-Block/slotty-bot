...
const ticketChannelMessageLink =
...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option.setName("key").setDescription("The key you want to send").setRequired(true)
  )
  .addUserOption((option) =>
    option
      .setName("user")
      .setDescription("The user you want the key to be sent to")
      .setRequired(true)
  );
...
export async function execute(interaction: CommandInteraction) {
  ...
  if (!license) {
    ...
    return interaction.editReply({
      content: `An error occurred, please contact the support.\n${codeBlock(
        "ps",
        `[ERROR]: "API Error with key ${optionKey}"`
      )}`,
    });
  }
  ...
  if (dbKey) {
    ...
    return interaction.editReply({
      content: `An error occurred, please contact the support.\n${codeBlock(
        "ps",
        `[ERROR]: "The key ${optionKey} is already linked to another user"`
      )}`,
    });
  }
...
The key is automatically linked to your account and you can use ${inlineCode(
    "/license info"
  )} to see its status.
    ...
    .then(async () => {
      ...
      if (!updatedUser) {
        ...
        return interaction.editReply({
          content: `An error occurred, please contact the support.\n${codeBlock(
            "ps",
            `[ERROR]: "Could not link license key to user"`
          )}`,
        });
      }
...
`;
      ...
      return await interaction.editReply({
        content: `Direct message sent to ${userMention(optionUser.id)} with the key ${inlineCode(
          optionKey
        )}.`,
      });
    })
