...
export const data = new SlashCommandBuilder()
	...
	.addSubcommand((subcommand) => subcommand.setName('balance').setDescription('Check your wallet balance.'))
	...
	.addSubcommand((subcommand) =>
		subcommand
			.setName('buycoins')
			.setDescription('Buy slotted coins with your subtime. (1 day = 100 coins)')
			.addIntegerOption((option) =>
				option
					.setName('days')
					.setDescription('The amount of days of your subtime you want to trade for slotted coins.')
					.setRequired(true)
					.setMinValue(1)
					.setMaxValue(100)
			)
	...
	.addSubcommand((subcommand) =>
		subcommand
			...
			.addIntegerOption((option) =>
				option
					.setName('days')
					.setDescription('The amount of days you want to buy with slotted coins.')
					.setRequired(true)
					.setMinValue(1)
					.setMaxValue(100)
			)
...
async function daily(interaction: CommandInteraction) {
	try {
		...
		const lastDaily = user.transactions.find(
			(t) => t.type === 'daily' && t.createdAt > new Date(Date.now() - ONE_HOUR * 20)
		);
		...
		if (lastDaily) {
			...
			return interaction.reply({
				content: `You can claim your daily reward in ${time(date, TimestampStyles.RelativeTime)}`,
				flags: MessageFlags.Ephemeral,
			});
		}
...
async function transfer(interaction: CommandInteraction) {
	...
	try {
		...
		return interaction.reply(`You transferred ${amount} coins to ${userMention(to.id)}.`);
	} catch (error) {
...
async function setBaseBet(interaction: CommandInteraction) {
	...
	if (!canChangeBet) {
		logger.error('User is not allowed to change base bet while being in an active round of roulette');
		return interaction.reply({
			content: 'You cant change your basebet while being in an active round of roulette.',
			flags: MessageFlags.Ephemeral,
		});
	}

	logger.info(`Setting base bet for user: ${interaction.user.id} with amount ${amount}`);
	try {
		const user = await prisma.user.findFirst({
			where: { discordID: interaction.user.id },
			include: { wallet: true },
...
async function buycoins(interaction: CommandInteraction) {
	...
	logger.info(`Attempting to trade ${days} days of subtime for ${coins} coins for user: ${interaction.user.id}`);
	...
	try {
		...
		if (!user.activeKey) {
			...
			await prisma.$disconnect();
			return interaction.editReply({
				content: 'You need to have an active license key linked to your account to buy coins.',
			});
		}
		...
		if (licenseInfo.dateActivated !== null && (licenseInfo.daysLeft < 0 || licenseInfo.daysValid < 0)) {
		...
		if (licenseInfo.daysLeft < days || licenseInfo.daysLeft < 1) {
			...
			await prisma.$disconnect();
			return interaction.editReply(`You dont have enough sub time to buy ${days} days worth of coins.`);
		}
		...
		logger.info(`License edited successfully, adding coins to user wallet: ${coins}`);
		...
		newLicenseEndDate.setDate(newLicenseEndDate.getDate() + newLicense.daysValid);
		...
		prisma.$disconnect();
		return interaction.editReply('An error occured, please contact the support.');
	}
}
...
async function buySubtime(interaction: CommandInteraction) {
	...
	logger.info(`Attempting to buy ${days} days of subtime for ${coins} coins for user ${interaction.user.id}`);
	...
	try {
		...
		if (!user.activeKey) {
			...
			await prisma.$disconnect();
			return interaction.editReply({
				content: 'You need to have an active license key linked to your account to buy subtime.',
			});
		}
		// Check if user has enough coins
		if (user.wallet.balance < coins) {
			logger.error(`User does not have enough coins to buy ${days} days of subtime`);
			await prisma.$disconnect();
			return interaction.editReply({
		...
		logger.info(`License edited successfully, removing coins from user wallet: ${coins}`);
