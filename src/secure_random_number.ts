...
export class SecureRandomGenerator {
  ...
  constructor(currentCommitment: string | null = null, commitmentSalt: Buffer | null = null) {
    if (
      currentCommitment &&
      commitmentSalt &&
  ...
  public generateCommitment(): string {
    ...
    const commitmentData = Buffer.concat([this.commitmentSalt, Buffer.from(timestamp)]);
    this.currentCommitment = createHash("sha256").update(commitmentData).digest("hex");
  ...
  private async gatherEntropy(): Promise<EntropyData> {
    try {
      ...
      throw new Error(
        `Entropy gathering failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
