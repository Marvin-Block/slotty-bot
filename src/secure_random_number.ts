...
export class SecureRandomGenerator {
    ...
    constructor(
        vdfIterations: number = 10000,
        currentCommitment: string | null = null,
        commitmentSalt: Buffer | null = null
    ) {
        this.vdfIterations = vdfIterations;

        if(currentCommitment && commitmentSalt && this.verifyCommitment(currentCommitment, commitmentSalt)) {
    ...
    private vdf(input: Buffer | string, iterations: number): string {
        let x = bigInt(Buffer.isBuffer(input) ? input.toString('hex') : input, 16).mod(this.PRIME_2048);
    ...
    private gatherSystemEntropy(): Buffer {
        ...
        return createHash('sha256')
            .update(cpuUserEntropy + cpuSystemEntropy + timeEntropy + memEntropy + timestampEntropy)
            .digest();
    }
    ...
    private async gatherEntropy(): Promise<EntropyData> {
        try {
            ...
            const current = await axios.get<DrandResponse>('https://api.drand.sh/public/latest');
            const previous = await axios.get<DrandResponse>(`https://api.drand.sh/public/${current.data.round - 1}`);
            ...
            throw new Error(`Entropy gathering failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }

    public async generateSecureRandom(min: number, max: number): Promise<RandomResult> {
        try {
            ...
            const vdfResult = this.vdf(combinedEntropy, this.vdfIterations);
    ...
    protected storeAuditRecord(record: AuditRecord): void {
        // Store in DB
